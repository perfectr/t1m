...
public class AuthenticationFailureBlocker {
    ...
    /**
     * This method will watch and count the number of failed authentication requests. If too many failed authentication requests occur within a specified
     * time period then this service will operate in a "go-slow" mode and delay responding by a specified time period.
     *
     * <p>This technique doesn't worry about IP addresses and will delay everyone on the system but valid users will still be able to login ok. It also doesn't
     * lock accounts because that creates an Support desk overhead with people needing to get accounts unlocked and in a worse case scenario it would be
     * relatively easy for an attacker to lock everyone's account and so create a nightmare for the support desk.</p>
     */
    public void trackFailedAuthentications(boolean currentRequestAuthenticatedOk) {
        ...
        if(currentFailureCount > authenticationFailureThreshold) {
            try { Thread.sleep(authenticationFailureTimeDelayInSecs * 1000); } catch (InterruptedException ex) {}
        }
    }
